    ...
    const String & projection_name)
{
    ...
    for (const auto & child : declared_columns.children)
    {
        ...
        {
            auto declared_type = DataTypeFactory::instance().get(type_ast);
        ...
        /// Preprocessed only, never sanity-checked; see `validateDeclaredColumnCodecs`.
        if (auto codec_ast = column_declaration.getCodec())
            codecs.emplace(
                column_name,
                CompressionCodecFactory::instance().validateCodecAndGetPreprocessedAST(
                    codec_ast, column_in_projection->type, /*sanity_check=*/false, /*allow_experimental_codecs=*/true));
    }
    ...
    bool attach_short_syntax)
{
    ...
    else
    {
        ...
        /// Substitute codec arguments into the stored definition, as `ColumnsDescription` does for a
        /// table's own columns, so both print `Delta(8)`. Applied to the owned clone, never the caller's
        /// AST. Idempotent, because `ReplicatedMergeTreeTableMetadata` re-derives projections from this
        /// definition on every `ALTER`.
        if (projection_definition->columns)
        {
    ...
    const IAST * declared_columns)
{
    ...
    if (declared_columns)
        declared_codecs = resolveDeclaredProjectionColumnCodecs(*declared_columns, result.sample_block, columns, result.name);
    ...
    for (const auto & column_with_type_name : result.sample_block)
    {
        ...
        else
        {
            ...
            if (auto declared_codec = declared_codecs.find(column_with_type_name.name); declared_codec != declared_codecs.end())
