...
-- ZooKeeper one, re-parsed and re-serialised, so the replicas below agree only if the codec-argument
...
-- An `ALTER` that leaves projections alone still re-derives each one from its stored - by now
-- substituted - definition. That must not substitute a second time.
